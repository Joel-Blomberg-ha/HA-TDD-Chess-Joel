...
import static ax.ha.tdd.chess.engine.pieces.PieceType.BISHOP;

public class Bishop extends ChessPiece{
    public Bishop(Player player, Coordinates location) {
        super(BISHOP, player, location);
    }
    ...
    public boolean canMove(Chessboard chessboard, Coordinates destination) {
        ...
        {
            if((((location.getX() + location.getY())% 2 == 0)&&((destination.getX()+destination.getY())% 2 == 0))|| (((location.getX() + location.getY())% 2 != 0)&&((destination.getX()+destination.getY())% 2 != 0))) {
